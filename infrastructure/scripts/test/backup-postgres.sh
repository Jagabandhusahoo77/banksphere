#!/bin/bash
# BankSphere TEST Postgres backup — runs ON the TEST EC2 instance (e.g. via
...
# lives on (see infrastructure/terraform/environments/test/ec2.tf's
...
BACKUP_FILE="${BACKUP_DIR}/banksphere-test-${TIMESTAMP}.sql.gz"
RETENTION_DAYS=7
...
find "${BACKUP_DIR}" -name 'banksphere-test-*.sql.gz' -mtime "+${RETENTION_DAYS}" -delete
