...
module "k3s" {
  ...
  namespace    = "${var.project_name}-${var.environment}" # banksphere-test — see the task's own required namespace naming
...
module "ec2" {
  ...
  ecr_repository_arns       = [for repo in data.aws_ecr_repository.this : repo.arn]
  ssm_parameter_path_prefix = local.ssm_parameter_path_prefix
