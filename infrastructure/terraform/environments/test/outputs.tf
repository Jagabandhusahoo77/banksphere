...
output "ecr_repository_urls" {
  description = "Same repositories DEV owns (see ecr.tf) — shown here for convenience, not created by this environment's state."
  value       = { for name, repo in data.aws_ecr_repository.this : name => repo.repository_url }
}

output "fqdns" {
  description = "The app-test/ops-test/api-test hostnames this environment expects to serve, once domain_name is configured."
