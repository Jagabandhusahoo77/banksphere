...
public class GlobalExceptionHandler {
     ...
     * BeneficiaryController) would be caught by this class's own
    ...
    public ResponseEntity<ErrorResponse> handleNotFound(BeneficiaryNotFoundException ex, HttpServletRequest request) {
        return ResponseEntity.status(HttpStatus.NOT_FOUND)
                .body(ErrorResponse.of(HttpStatus.NOT_FOUND.value(), "Not Found", ex.getMessage(), request.getRequestURI()));
    }

    @ExceptionHandler(BeneficiaryAccessDeniedException.class)
    public ResponseEntity<ErrorResponse> handleAccessDenied(BeneficiaryAccessDeniedException ex, HttpServletRequest request) {
        return ResponseEntity.status(HttpStatus.FORBIDDEN)
    ...
    public ResponseEntity<ErrorResponse> handleDuplicate(DuplicateBeneficiaryException ex, HttpServletRequest request) {
    ...
    public ResponseEntity<ErrorResponse> handleDataIntegrityViolation(DataIntegrityViolationException ex, HttpServletRequest request) {
        // The concurrency backstop for handleDuplicate above: two requests
        // can both pass the application-layer existence check before either
        // commits, so the database's partial unique index is the real
        // guarantee — this is what turns that constraint violation into a
