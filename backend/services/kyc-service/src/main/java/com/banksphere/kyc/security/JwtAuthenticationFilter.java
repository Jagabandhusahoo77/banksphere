 ...
 * plain {@code String}, never wrapped) — see {@link CurrentUser}. Runs
 * alongside (never in place of) {@link EmployeeJwtAuthenticationFilter};
 * since the two signing keys are cryptographically distinct, a token can
 * only ever satisfy one filter.
 */
@Component
